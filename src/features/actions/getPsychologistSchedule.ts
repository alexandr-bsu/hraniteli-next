...
// Отправка анкеты
export const submitQuestionnaire = async (formData: IApplicationFormData) => {
    try {
...
export const getFilteredPsychologists = async () => {
    try {
        const response = await fetch('https://n8n-v2.hrani.live/webhook/get-filtered-psychologists-test-contur', {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
            }
